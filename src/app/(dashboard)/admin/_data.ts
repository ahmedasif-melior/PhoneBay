...
export async function getAdminData(): Promise<AdminData> {
  ...
  try {
    ...
    const [
      ...
      supabase.from("shop_profiles").select("id", { count: "exact", head: true }).neq("verified", true),
      supabase.from("shop_profiles").select("id", { count: "exact", head: true }).eq("verified", true),
