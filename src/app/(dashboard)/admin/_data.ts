...
export function getAdminData() {
  ...
  const pipeline: Record<string, StageItem[]> = {
    users: [
    ...
    opportunities: [
      { label: "New signups", value: Number((db.prepare("SELECT COUNT(*) as count FROM users WHERE created_at >= datetime('now', '-30 days')").get() as { count: number }).count) },
      { label: "Qualified leads", value: Number((db.prepare("SELECT COUNT(*) as count FROM users WHERE role IN ('USER', 'SHOP')").get() as { count: number }).count) },
      { label: "Live listings", value: Number((db.prepare("SELECT COUNT(*) as count FROM listings WHERE status = 'active'").get() as { count: number }).count) },
      { label: "Potential sales", value: Number((db.prepare("SELECT COUNT(*) as count FROM listings WHERE price >= 50000").get() as { count: number }).count) },
      { label: "Completed orders", value: Number((db.prepare("SELECT COUNT(*) as count FROM orders WHERE status = 'delivered'").get() as { count: number }).count) },
    ],
  };
