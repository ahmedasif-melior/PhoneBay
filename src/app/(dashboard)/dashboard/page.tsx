...
export default async function DashboardOverviewPage() {
  ...
  const [
    ...
    /*
     * Only use certificate columns that this dashboard actually needs.
     *
     * IMPORTANT:
     * We are NOT querying:
     *   overall_score
     ...
     *   issued_at
     *
     * because those columns do not exist in your current table.
     */
    db
      .from("certificates")
      .select(
  ...
  const recentListing = recentListings[0] ?? null;

  const savedPhones = savedListingsResult.error
    ? 0
    : savedListingsResult.count ?? 0;
  ...
  if (savedListingsResult.error) {
    console.error(
      "[DASHBOARD] Failed to load saved listings count:",
      savedListingsResult.error.message,
    );
  }

  if (ordersResult.error) {
    console.error(
      "[DASHBOARD] Failed to load orders count:",
      ordersResult.error.message,
    );
  }

  if (recentVerificationResult.error) {
    console.error(
      "[DASHBOARD] Failed to load recent verification:",
      recentVerificationResult.error.message,
    );
  }
  ...
  const verification =
    recentVerificationResult.data as VerificationRow | null;

  const verificationListing = Array.isArray(
    verification?.listings,
  )
    ? verification.listings[0]
    : verification?.listings;

  const recentVerification =
    verification && verificationListing
      ? {
          model: verificationListing.model,
          certificate_id: verification.id,
        }
      : null;
  ...
  return (
    <div className="max-w-5xl">
      ...
      <p className="text-ink-soft mt-1">
        {user.accountPurpose
          ? `You’re set up as a ${user.accountPurpose} on PhoneBay.`
          : "Complete your profile to set your buyer or seller preference."}
      </p>
        ...
        <Card>
          ...
          {recentVerification ? (
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-ink">
                  {recentVerification.model}
                </p>

                <p className="text-sm text-ink-soft">
                  Certificate {recentVerification.certificate_id}
                </p>
              </div>
